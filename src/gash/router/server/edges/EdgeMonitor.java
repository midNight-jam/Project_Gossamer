...
public class EdgeMonitor implements EdgeListener, Runnable {
	...
	public void run() {
		while (forever) {
			System.out.println("I'm in edge Monitor !!");
			try {
				Logger.DEBUG("Number of outbound edges : " + this.outboundEdges.map.size());
				
					System.out.println(this.outboundEdges.map.values().size());
	...
	public synchronized void onAdd(EdgeInfo ei) {
		try {
			...
			Logger.DEBUG("Before connecting to node : " + ei.getRef());
			EventLoopGroup group = new NioEventLoopGroup();
			Bootstrap b = new Bootstrap();
			...
			// Make the connection attempt.
			ChannelFuture cf = b.connect(ei.getHost(), ei.getPort() - 1).syncUninterruptibly();
			if(!cf.isDone() || !cf.isSuccess()){
				Logger.DEBUG("Failed to establish channel !");
			}
