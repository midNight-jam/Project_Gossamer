...
import gash.router.server.DiscoveryServer;
...
public class QueueServer {
	...
	static QueueCommandInit commInit;
	/**
	...
	public QueueServer(File cfg) {
		init(cfg);
		commInit = new QueueCommandInit(conf, false, leaderMessageQue, nonLeaderMessageQue);
	}
	
	public static QueueCommandInit getCommandInit(){
		return commInit;
	}
	...
	public void startServer() {
		...
		//QueueCommandHandler qch = QueueServer.getCommandInit().getCommandHandler();
		startDiscoveryServer();
		logger.info(" (@>@) Queue Server started");
		try{
			  getLocalhostJedis().select(0);// uses default database 
			  //setting the queserver ip and port in global redis cluster directory
			  getLocalhostJedis().set("1","192.168.1.10"+":4068");// use this to set leader on redis, 
	...
	public void startDiscoveryServer(){
		logger.info(" (D.D) Discovery Server started");
		DiscoveryServer discoveryServer = new DiscoveryServer(QueueServer.getCommandInit().getCommandHandler());
		Thread serverThread = new Thread(discoveryServer);
		serverThread.start();
	}
	...
	private static class StartCommandCommunication implements Runnable {
		...
		public void run() {
			...
			//commInit = new QueueCommandInit(conf, compressComm, leaderMessageQue, nonLeaderMessageQue);
